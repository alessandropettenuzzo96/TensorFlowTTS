    ...
    for t in ["train", "valid"]:
        ...
        for i in tqdm(os.listdir(os.path.join(pre_path, "ids"))):
            if use_norm == "t":
                mel = np.load(
                    os.path.join(
                        pre_path, "norm-feats", f"{i.replace('-ids.npy', '')}-norm-feats.npy"
                    )
                )
            else:
                mel = np.load(
                    os.path.join(
                        pre_path, "raw-feats", f"{i.replace('-ids.npy', '')}-raw-feats.npy"
                    )
                )
            ...
            np.save(
                os.path.join(pre_path, "fix_dur", f"{i.replace('-ids.npy', '')}-durations.npy"),
